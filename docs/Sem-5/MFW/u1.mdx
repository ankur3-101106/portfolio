...
#### 5.3.1.1 HTTP Response Status Codes
...
#### 5.3.3.1 Same-Origin Policy
The same-origin policy is a web browser security mechanism that aims to prevent websites from attacking each other. It permits scripts contained in a first web page to access data in a second webpage, but only if both web pages have the same origin (URI scheme, domain, and port number). Without this, a malicious website could read your emails or private messages from other sites you are logged into.
...
- **Session Cookie:** Used to store information temporarily and are deleted when the user closes their browser.
...
- **Third-Party Cookie:** Set by a domain other than the one the user is visiting, often used by advertisers and analytics providers.
...
- **Spider:** A web spider/crawler used to map the target web application to get a list of endpoints.
...
- **Extender:** Supports external components (BApps) to be integrated into the tools suite to enhance its capabilities.
- **Scanner:** Automatically scans the website for many common vulnerabilities (not available in the community edition).
